...
| 想做什么 | 怎么做 |
| --- | --- |
| 移动玻璃 | `Alt + 鼠标左键` 在玻璃任意位置拖动；或按住玻璃四周 12px 的空白圈拖动（无需 Alt） |
| 调整大小 | 鼠标贴到玻璃边缘约 6px，光标变缩放箭头后拖动 |
| 右键菜单 | 设置… ／ 回正位置 ／ 隐藏 ／ 退出（只关这一块玻璃） |
...
- **外观**（对所有玻璃全局生效）：玻璃颜色、不透明度 10%–100%、字体颜色、字体、字号
- **行为**：穿透热键（点击输入框后直接按组合键）、锁定位置（禁止拖动与缩放）、开机自启

不透明度作用于整块玻璃，文字会随之变淡——这是"整块半透明"方案的固有行为。
...
| `Appearance.OpacityPercent` | 不透明度（10–100） |
| `Appearance.FontName` / `FontSize` | 字体 / 字号 |
