...
- 浮层直接嵌入任务栏窗口，任务栏移动、自动隐藏、DPI 变化或资源管理器重启后都会自动跟随恢复；全屏应用时随任务栏一起消失
- 默认位置自动避开任务栏上已有的图标与工具窗口，挑靠近中间的空隙摆放；左右拖动即可微调，支持鼠标穿透、背景/文字颜色与不透明度独立设置
...
- **位置**：默认**自动避开任务栏上已有的内容**（托盘图标、任务按钮、TrafficMonitor 等任务栏工具），从空隙里挑最靠近中间的位置摆放，并且每秒跟随任务栏布局变化重新避让；设置页可固定为"系统托盘左侧 / 任务栏最左侧"，也可以在任务栏上按住文字左右拖动微调，右键菜单提供回正与隐藏；勾选"鼠标穿透"后整块区域点击穿透、位置固定
- **外观**：背景颜色/不透明度、文字颜色/不透明度、字号（7–24pt）独立于玻璃设置；字体跟随玻璃字体
- **自适应**：行数越多字号越小（有可读下限），保证所选行完整放进任务栏高度；实在放不下时从起始行开始只显示放得下的行数；超宽的行自动截断为省略号
- **稳定跟随**：浮层嵌入任务栏窗口本身，任务栏自动隐藏、全屏播放、移动到竖屏、改 DPI 或重启资源管理器后都会自动恢复，不会悬浮在全屏画面上
...
- **任务栏**：在任务栏显示待办、显示行范围（第 N–M 行）、背景颜色与不透明度、文字颜色与字号、鼠标穿透任务栏显示
...
| `Taskbar.BackgroundColor` / `BackgroundOpacityPercent` | 任务栏底色（默认 `#1F1F1F`）与不透明度 0–100 |
| `Taskbar.TextColor` / `TextOpacityPercent` | 文字颜色（默认白色）与不透明度 0–100 |
| `Taskbar.FontSize` | 任务栏字号 7–24pt，字体族跟随玻璃字体 |
